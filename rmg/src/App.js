...
const INIT_STATE = {
  columns: [{id: 1 , selected: false, a: 1, b: 0}, {id: 2,selected: false, a: 1, b:0},
...
const App = () => {
  ...
  const handleDotClick = useCallback(
    id => evt => {
      synth1.triggerAttackRelease('2n')
      dispatch({type: TYPES.SELECT, payload: id})
    }, [dispatch])
const handleAddClick = useCallback(
  () => dispatch({type: TYPES.ADD_COLUMN}), [dispatch]);
     ...
     <rect width="100%" height="100%" fill={colors.color1} />
{
  ...
  <>
    ...
    <circle onClick={handleDotClick(item.id)} cx={item.x}
     cy="100" r="30" 
     fill={item.selected === true ? colors.color4 : colors.color5} />
     <line x1={item.x} x2={item.x} y1='0' y2={height} stroke={colors.color3}/>
    </>
)
...
style={{fill: colors.color4, position:'absolute', top: '5px', right:'5px'}} />
</>
)
};
